...
import type { ColumnDef, DataDensity } from '@/components/MrxGrid'
...
| \`<MrxGridSmartToolbar>\` | Toolbar batteries-included | 1 component |
| Plugins (\`MrxGridPlugin\`) | Behavior cross-cutting | 1 fonction |
| \`<MrxColumn>\` declarative | Definition de colonnes en SFC | template |

### Plugins API

\`\`\`ts
interface MrxGridPlugin {
  name: string
...
\`\`\`vue
<MrxGrid persist-key="my-grid-v1" :columns="columns" :rows="rows" />
\`\`\`
    ...
    template: `
      <div class="sb-mrx-shell">
        ...
        <div class="sb-mrx-frame">
          <MrxGrid :height="560" :columns="lmColumns" :rows="lmProducts" persist-key="lm-products-v1" />
        </div>
      </div>
    `,
  }),
...
import { ref } from 'vue'
import { MrxGrid, MrxGridToolbar, MrxTableMenuDrawer } from '@/components/MrxGrid'
...
Utilisez \`<MrxGridSmartToolbar>\` qui bundle ce wiring (voir story *Devtools / Event console*).
        `,
      },
    },
  },
  render: () => ({
    components: { MrxGrid, MrxGridToolbar, MrxTableMenuDrawer },
    setup() {
      const settingsOpen = ref(false)
      const fullscreen = ref(false)
      const density = ref<DataDensity>('default')
    ...
    template: `
      <div class="sb-mrx-shell">
        ...
        <p><code>MrxGridToolbar</code> est rendu via le slot <code>#toolbar</code> du grid : il reste donc visible quand on passe en plein écran. Le drawer ferme via la croix ou en cliquant l'overlay.</p>
        <div class="sb-mrx-frame">
          <MrxGrid :height="560"
            :columns="lmColumns"
            :rows="lmProducts"
...
\`\`\`ts
const urlSyncPlugin: MrxGridPlugin = {
  name: 'url-sync',
  init({ state }) {
    const stop = watch(
...
\`\`\`vue
<MrxGrid :plugins="[urlSyncPlugin]" :columns="cols" :rows="rows" />
\`\`\`
  ...
  render: () => ({
    ...
    template: `
      <div class="sb-mrx-shell">
        ...
        <div class="sb-mrx-frame">
          <MrxGrid :height="560" :columns="lmColumns" :rows="lmProducts" :plugins="plugins" />
        </div>
      </div>
    `,
  }),
...
\`\`\`vue
<MrxGrid :columns="cols" :rows="rows" history-id="my-grid" />
\`\`\`
...
\`\`\`ts
import { useUndoRedoPlugin } from '@/components/MrxGrid'
...
export const DeclarativeColumns: Story = {
  name: '<MrxColumn> declarative API',
  parameters: {
    docs: {
      description: {
        story: `
## Declarative \`<MrxColumn>\` children
...
\`\`\`vue
<MrxGrid :rows="rows" selectable>
  <MrxColumn field="sku" header-name="Réf" :width="120" pinned="start" />
  <MrxColumn field="name" header-name="Produit" :width="260" sortable filterable filter-type="text" />
  <MrxColumn field="price" header-name="Prix" :width="110" sortable editable cell-editor="number">
...
\`\`\`vue
<MrxGrid :columns="propCols" :rows="rows">
  <MrxColumn field="custom" header-name="Custom">
    <template #cell="{ row }">…</template>
  </MrxColumn>
</MrxGrid>
\`\`\`

Quand un \`field\` matche entre les deux APIs, le \`<MrxColumn>\` enfant **override** la version prop. Pratique pour ajouter un slot sans réécrire la \`ColumnDef\`.

### Sous le capot

Chaque \`<MrxColumn>\` s'enregistre via \`provide(MRX_COLUMN_REGISTRY_KEY)\`. \`MrxGrid\` merge avec \`props.columns\` pour produire \`mergedColumns\` (le registry gagne en cas de collision sur \`field\`).
        `,
      },
    },
  },
  render: () => ({
    components: { MrxGrid, MrxColumn },
    setup: () => ({ lmProducts }),
    template: `
      <div class="sb-mrx-shell">
        <h2>Declarative columns via <code>&lt;MrxColumn&gt;</code></h2>
        ...
        <div class="sb-mrx-frame">
          <MrxGrid :height="560" :rows="lmProducts">
            <MrxColumn field="sku" headerName="Réf" width="120px" pinned="start" />
            <MrxColumn field="name" headerName="Produit" width="260px" :editable="true" />
            <MrxColumn field="brand" headerName="Marque" width="140px" :sortable="true" />
            <MrxColumn field="price" headerName="Prix" width="110px" :sortable="true" :editable="true" />
            <MrxColumn field="store" headerName="Magasin" width="180px" pinned="end" />
          </MrxGrid>
        </div>
      </div>
    `,
