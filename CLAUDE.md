...
Unit tests live in two places: `src/__tests__/` (Vue/composable tests) and `src/components/MrxGrid/features/__tests__/` (engine tests).
...
1. **Legacy composables** (`src/composables/use*.ts`) — owns most of the runtime today. `MrxGrid.vue` imports ~20 of these directly and wires them by hand. Each composable holds its own local state.
2. **Engine layer** (`src/components/MrxGrid/{state,engine,features,models}/`) — the Angular-parity port. Central `GridState` + `useGridEngine()` orchestrates a pure-computed pipeline:
   `sourceData → sortedData → filteredData → paginatedData → displayRows`
   Each `*Engine.ts` reads/writes `GridState` instead of owning local state. New features and refactors land here. `MrxGrid.vue` already creates a `GridState` and calls `useGridEngine()` alongside the legacy composables — they're wired in parallel and both write to the same DOM.
...
### Component hierarchy (`src/components/MrxGrid/`)

```
MrxGrid.vue                 root orchestrator (props/events, provides GridState + ColumnRegistry)
MrxColumn.vue               declarative column API — registers via MRX_COLUMN_REGISTRY_KEY
components/
  header/                   MrxGridHeader, MrxGridHeaderCell, MrxGridHeaderMenu, MrxGridFilterRow,
                            MrxGridFilterCell, MrxColumnFilterOverlay, MrxGridGroupBar,
                            MrxGridHiddenBar, MrxGridSpreadsheetHeader, MrxGridTagBar
  body/                     MrxGridBody, MrxGridRow, MrxGridCell, MrxGridGroupRow,
                            MrxGridDetailRow, MrxGridFormulaEditor, MrxGridEmptyState
  footer/                   MrxGridFooter, MrxGridLoadingIndicator
...
`index.ts` is the barrel — consumers `import { MrxGrid, ... } from '@/components/MrxGrid'`. It also re-exports `useGridState`, `useGridEngine`, `defineStatusRenderer`, formula APIs, and the plugin contract.
...
Legacy feature implementations wired into `MrxGrid.vue`:
...
Engine equivalents under `src/components/MrxGrid/features/` end in `Engine.ts` (e.g. `useSortEngine`, `useFilterEngine`, `useCellSelectionEngine`, `useFormulaEngine`, …) plus formula sub-package and `renderers/` (built-in cell renderers + `defineStatusRenderer`).

### Key invariants

- **GridState provide/inject** — `MrxGrid.vue` calls `useGridState()` and provides under `GRID_STATE_KEY`. Sub-components read it via `useGridContext()` (alias `injectGridState` for back-compat).
- **Column registry** — `MrxColumn` children register via `MRX_COLUMN_REGISTRY_KEY`. When both `:columns` prop and `<MrxColumn>` children are provided, the registry overrides the prop on matching `field`.
...
- **Plugin contract** — `<MrxGrid :plugins="[plugin]" />`. Each plugin receives `{ state, engine }` on `init` and returns a cleanup function.
...
- **Density row heights** (must match SCSS padding): `compact: 32`, `default: 48`, `comfortable: 64` — defined in `MrxGrid.vue` as `DENSITY_ROW_HEIGHT`. (The legacy `defaults.ts` constant uses different numbers for the engine layer; treat the `MrxGrid.vue` constants as authoritative for visual rendering.)
...
- **Scoped CSS** in SFCs; shared SCSS partials in `src/components/MrxGrid/styles/`
