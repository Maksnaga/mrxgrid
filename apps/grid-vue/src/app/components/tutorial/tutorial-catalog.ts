/**
 * Catalogue du tutoriel "Construire une datagrid pro avec Grid".
 *
...
export const TUTORIAL_STEPS: TutorialStep[] = [
  ...
  // ===========================================================================
  {
    ...
    title: 'Installer et importer Grid',
    objective: 'Mettre en place les dépendances pour utiliser Grid dans un projet Vue 3.',
    explanation: [
      'Grid est packagé en ESM Vue 3 + TypeScript et a deux dépendances peer : <code>vue@^3.5</code> et <code>@mozaic-ds/vue</code> (le design system Mozaic d\'Adeo qui fournit les inputs, modals, drawers, etc.). Toutes les exports publics sont accessibles depuis le barrel <code>@/components/Grid</code>.',
      'Pour ce demo, on importe le composant principal <code>Grid</code> et le type <code>ColumnDef</code> qui décrit chaque colonne. Le reste (types CellRendererProps, renderers built-in, filtres, plugins, etc.) est exposé au même niveau.',
    ],
...
<template>
  ...
  },
  {
    ...
    snippet: `import type { ColumnDef } from '@/components/Grid'
...
const columns: ColumnDef[] = [
  ...
  },
  {
    ...
    width: '110px',
    sortable: true,
    cellClass: 'grid-cell-num', // classe CSS appliquée à la cellule
  },
  {
    field: 'status',
    headerName: 'État',
  ...
  },
  {
    ...
    explanation: [
      ...
      'La virtualisation 2D (vertical + horizontal) est toujours active sous le capot — rien à brancher, le grid encaisse 100k+ lignes / 200+ colonnes sans config.',
    ],
    snippet: `<script setup lang="ts">
import { ref } from 'vue'
...
/>`,
  ...
  },
  {
    ...
    explanation: [
      'Quand les éditeurs built-in ne suffisent pas, déclare <code>cellEditor: \'custom\'</code> sur la colonne et fournis le composant via le slot <code>#edit-{field}</code> sur <code>&lt;Grid&gt;</code>. Le scope du slot expose : <code>value, editValue, updateValue, commit, cancel, setValue, row, field, rowIndex, column</code>.',
...
/>`,
  ...
  },
  {
    ...
    snippet: `<ad-grid-vue :columns="columns" :rows="rows">
...
<template>
  ...
  },
  {
    ...
    explanation: [
      ...
      'Le pattern courant : un mini-grid imbriqué (mouvements de stock, historique, sous-éléments). Grid se réutilise lui-même en read-only.',
    ],
    snippet: `<ad-grid-vue
  :columns="columns"
  :rows="rows"
...
/>`,
  ...
  },
  {
    ...
    explanation: [
      'Le slot <code>#toolbar</code> remplace la toolbar par défaut. Tu peux y poser <code>&lt;ad-grid-toolbar&gt;</code> qui expose en one-liner les features built-in (<code>fullscreen, settings, filters, group, keyboard, export</code>) et entourer avec tes propres éléments.',
