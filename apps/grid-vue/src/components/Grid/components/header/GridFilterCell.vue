 ...
 *
 * Single-responsibility cell rendered three times by `AdGridFilterRow`
 * (left-pinned / center / right-pinned). Resolves the renderer in this
 * priority order:
 *
 *   1. `#filter-{field}` / `<ad-grid-column> #filter` slot
 ...
 * branches that previously lived inline in `AdGridFilterRow.vue`.
 */
...
import { injectGridSlots, resolveFilterSlot } from '../../state/GridSlots'

defineOptions({ name: 'AdGridFilterCell' })
...
// FilterConfig (`component`, `doesFilterPass`) is a builder/overlay
...
<template>
  ...
  <template v-else-if="inlineFilter">
    ...
    <div v-else-if="inlineFilter.type === 'date'" class="grid-filter-cell__date-range">
      <MDatepicker
        :id="`grid-filter-${column.field}-from`"
        size="s"
        :model-value="dateRange.from"
        @update:modelValue="onDateFrom"
      />
      <MDatepicker
        :id="`grid-filter-${column.field}-to`"
        size="s"
