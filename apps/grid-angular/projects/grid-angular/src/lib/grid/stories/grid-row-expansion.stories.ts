...
\`[expandable]="true"\` ajoute un chevron par ligne ; le contenu déplié est fourni par \`[detailTemplate]\` (sinon un détail par défaut). L'état d'expansion est keyé par \`rowIdField\` — il survit au tri, à la pagination et au remplacement des instances — et chaque toggle émet \`(rowExpand)\`.
...
Les detail rows s'étendent sur toute la largeur de la grille (elles ne scrollent pas avec les colonnes) et cohabitent avec les colonnes pinnées et la sélection.
        `,
      },
    },
  },
